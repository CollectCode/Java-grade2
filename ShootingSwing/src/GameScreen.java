...
public class GameScreen extends JPanel {
    ...
    public Image title, title_key;//타이틀화면
    ...
    GameScreen(ShootSwing _main){                            
            //this.main=(ShootSwing)this.getParent();
                         this.main = _main;
    ...
    public void paint(Graphics g){
            if(gc==null) {
                    dblbuff=createImage(
                                                        main.gScreenWidth,main.gScreenHeight);//더블 버퍼링용 오프스크린 버퍼 생성. 필히 paint 함수 내에서 해 줘야 한다. 그렇지 않으면 null이 반환된다.
    ...
    public void update(Graphics g){//화면 깜박거림을 줄이기 위해, paint에서 화면을 바로 묘화하지 않고 update 메소드를 호출하게 한다.
            ...
            System.out.println("In GameScreen.update, cnt = "+ cnt + ", gamecnt="+gamecnt);
            if(gc==null) return;
    ...
    public void Draw_BG2(){
            int i;
            System.out.println("bg_f.getWidth(null) = " + bg_f.getWidth(null) );
            step=(cnt%(bg_f.getWidth(null)/main.scrspeed))*main.scrspeed;
            gc.drawImage(bg_f,0-step,540-bg_f.getHeight(null)+v[(cnt/20)%8]*2,this);
            //System.out.println("요기"+step);
            if(step>=bg_f.getWidth(this)-main.gScreenWidth) {
                    gc.drawImage(bg_f,0-step+bg_f.getWidth(null),540-bg_f.getHeight(this)+v[(cnt/20)%8]*2,this);
            }
